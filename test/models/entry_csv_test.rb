...
describe EntryCsv do
  it 'generates CSV' do
    ...
    generate_csv(entries).tap do |csv|
      ...
      csv.each_with_index do |row, index|
        ...
        assert_equal "01:#{expected_minutes}", row['duration']
  ...
  it 'generates CSV with rounded values' do
    adrien = create_user(name: 'adrien')
    louis = create_user(name: 'louis')
    ...
    entries = [
      build_entry(user: adrien, project: client1_project1, title: "Adrien entry #1", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: adrien, project: client1_project1, title: "Adrien entry #2", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: louis, project: client1_project1, title: "Louis entry #1", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: louis, project: client2_project1, title: "Louis entry #2", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: louis, project: client2_project1, title: "Louis entry #3", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: adrien, project: no_client_project1, title: "Adrien entry #3", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: adrien, project: no_client_project1, title: "Adrien entry #4", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: louis, project: no_client_project1, title: "Louis entry #4", started_at: now, stopped_at: now + 1.hour + 2.minutes),
      build_entry(user: louis, project: no_client_project1, title: "Louis entry #5", started_at: now, stopped_at: now + 1.hour + 2.minutes)
    ]
    generate_csv(entries, rounded: true).tap do |csv|
      assert_equal entries.length, csv.length
      csv.each_with_index do |row, index|
        assert_equal entries[index].user.name, row['user']
        if row['client']
        ...
        assert_equal entries[index].title, row['title']
        expected_minutes = "%02.f" % (index+2)
        assert_equal "01:05", row['duration']
        assert_equal entries[index].rounded_started_at.strftime("%d/%m/%Y"), row['date']
        assert_equal entries[index].rounded_started_at.strftime("%H:%M"), row['start time']
        assert_equal entries[index].rounded_stopped_at.strftime("%H:%M"), row['end time']
     end
    end
  end
